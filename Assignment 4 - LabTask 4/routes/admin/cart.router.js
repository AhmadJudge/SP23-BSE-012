...
const FlashMessage = require('../../models/flashMessage.model')
...
router.post('/confirmation', async (req, res) => {
  ...
  if (!name || !email || !address || !paymentMethod) {
      ...
      return res.redirect('/confirmation'); // Redirect to show the error message
  }
  ...
  try {
      ...
      res.cookie('cart', JSON.stringify({}), { maxAge: 0 }); // Clear the cart
      req.session.success_msg = 'Your order has been placed successfully!';
      res.redirect('/confirmation');
  } catch (error) {
      console.error('Checkout Error:', error);
      req.session.error_msg = 'Something went wrong. Please try again.';
      res.redirect('/confirmation'); // Redirect to show the error message
  }
});
...
//       success_msg,
//       error_msg,
//       message: null, // Keep `message` null if not needed for anything else
//   });
// });
